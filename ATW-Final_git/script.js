...
(function () {
  ...
  function tick() {
    ...
    if (phase === "grow" || phase === "spin") {
      const pct = Math.min(Math.round(loaderCount / LOADER_TOTAL * 100), 100);
      ctx.save();
      ctx.font = "bold 14px 'Gloria Hallelujah', cursive";
      ctx.fillStyle = "rgba(255,255,255,0.7)";
      ctx.textAlign = "right";
      ctx.fillText(pct + "%", w - 20, 30);
      ctx.restore();
    }

    if (phase === "grow") {
      smoothR += (DOT_MAX_R - smoothR) * 0.08;
      rot     += 0.01;
      ...
      if (Math.abs(smoothR - DOT_MAX_R) < 0.5) {
        smoothR = DOT_MAX_R;
        phase   = "spin";
      }
    } else if (phase === "spin") {
